import  StaffAdminLayout  from "../../layouts/StaffAdminLayout";
import { ClipboardList, Clock, CheckCircle } from "lucide-react";

export default function StaffDashboard() {
  const stats = [
    {
      label: "Assigned Grievances",
      value: "12",
      icon: ClipboardList,
      bgColor: "bg-blue-100",
      iconColor: "text-primary",
    },
    {
      label: "Pending Responses",
      value: "5",
      icon: Clock,
      bgColor: "bg-orange-100",
      iconColor: "text-orange-600",
    },
    {
      label: "Resolved Today",
      value: "2",
      icon: CheckCircle,
      bgColor: "bg-green-100",
      iconColor: "text-green-600",
    },
  ];

  return (
    <StaffAdminLayout>
      <h1 className="text-3xl font-bold mb-8">Staff Dashboard</h1>

      {/* Stats */}
      <div className="grid md:grid-cols-3 gap-6 mb-8">
        {stats.map((stat) => {
          const Icon = stat.icon;
          return (
            <div
              key={stat.label}
              className="bg-white rounded-lg shadow-md border p-6 hover:shadow-lg transition-shadow"
            >
              <div className="flex justify-between">
                <div>
                  <p className="text-sm text-muted-foreground mb-2">
                    {stat.label}
                  </p>
                  <p className="text-3xl font-bold">{stat.value}</p>
                </div>
                <div className={`${stat.bgColor} p-3 rounded-lg`}>
                  <Icon className={stat.iconColor} size={24} />
                </div>
              </div>
            </div>
          );
        })}
      </div>

      {/* Quick Actions */}
      <div className="bg-white rounded-lg shadow-md border p-8">
        <h2 className="text-xl font-bold mb-6">Quick Actions</h2>
        <div className="grid md:grid-cols-2 gap-4">
          <a
            href="/staff/grievances"
            className="flex justify-between p-4 border rounded-lg hover:bg-secondary/50"
          >
            <div>
              <p className="font-semibold">View Assigned Grievances</p>
              <p className="text-sm text-muted-foreground">
                Manage your assignments
              </p>
            </div>
            →
          </a>

          <div className="flex justify-between p-4 border rounded-lg opacity-60 cursor-not-allowed">
            <div>
              <p className="font-semibold">View Reports</p>
              <p className="text-sm text-muted-foreground">
                Performance analytics
              </p>
            </div>
            →
          </div>
        </div>
      </div>
    </StaffAdminLayout>
  );
}
